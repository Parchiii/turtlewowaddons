...
local function UnitXP_SP3_combatTextSP3(enable)
    local test, result = pcall(UnitXP, "combatTextSP3", enable);
...
local function UnitXP_SP3_combatTextSP3_fontSize(size)
    local test, result, reloadResult = pcall(UnitXP, "combatTextSP3", "setFontSize", size);
    ...
    if not reloadResult then
        if not skipUpdateMessage then
            skipUpdateMessage = true;
            UnitXP_SP3_Print(
                "UnitXP_SP3 failed to reload combat message font. This could because of missing d3dx9_43.dll on your system. Please try installing DirectX End-User Runtimes.");
        end
        return UnitXP_SP3_Addon["combatTextSP3_fontSize"];
    end
...
local function UnitXP_SP3_combatTextSP3_fontName(name)
    local test, result, reloadResult = pcall(UnitXP, "combatTextSP3", "setFontName", name);
...
local function UnitXP_SP3_combatTextSP3_nameplateHeight(size)
    local test, result = pcall(UnitXP, "combatTextSP3", "setNameplateHeight", size);
...
function UnitXP_SP3_OnEvent(event)
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        local test = pcall(UnitXP, "FPScap", UnitXP_SP3_Addon["FPScap"]);
        local test2 = pcall(UnitXP, "backgroundFPScap", UnitXP_SP3_Addon["backgroundFPScap"]);
    ...
    elseif event == "PLAYER_LEAVING_WORLD" then
        ...
        local test = pcall(UnitXP, "FPScap", 0);
        local test2 = pcall(UnitXP, "backgroundFPScap", 0);
